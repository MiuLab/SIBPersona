...
class MemoryManager():
    def __init__(self, first_order_memories, second_order_memories, top_order_memories, embeddings, 
                 model="gpt-4o", model_instance=None, tokenizer=None, language: str = "zh"):
        """
        ...
        Args:
            ...
            model: The model to use for all API requests (default: "gpt-4o")
            ...
            language: Language for prompts ("zh" or "eng")
        """
        ...
        chat_endpoint = os.environ.get('AZURE_ENDPOINT', '')
        # Embedding client always uses Azure (for now)
        self.embedding_client = OpenAI(api_key=chat_api_key, base_url=chat_endpoint)
        # Only set up chat client if not using open-source model
        if not is_open_source_model(self.model):
            if self.model.startswith("gpt"):
                
                self.client = OpenAI(api_key=chat_api_key, base_url=chat_endpoint)
            elif self.model.startswith("gemini-"):
                self.client = langchain_google_vertexai.ChatVertexAI(
    ...
    def _generate_chat_response(self, system_message: str, user_message: str, temperature: float = 0.3) -> str:
        """
        Generate a chat response using either OpenAI or open-source models.
        
        Args:
            system_message: The system message for the prompt
            user_message: The user message for the prompt
        ...
        if is_open_source_model(self.model) and self.model_instance is not None and self.tokenizer is not None:
            # print("Running open-source model inference...")
            return self._run_open_source_inference(messages, temperature)
        else:
            for _ in range(5):
                try:
                    ...
                    print(f"response generation error: {e}. Retrying...")
            return ""

    
    def _run_open_source_inference(self, messages: list, temperature: float = 0.3, max_tokens: int = 1024) -> str:
        """
        ...
        try:
            if "Olmo" in self.model:
                ...
                output = self.model_instance.generate(
                    ...
                    temperature=max(temperature, 0.1),  # Ensure temperature is positive
                ...
                return f"Error: Unsupported open-source model: {self.model}"
    ...
    def _check_cache(self, conversation):
        ...
        if similarity > 0.7:  # Lower threshold to consider more potential matches
            ...
            system_msg = "You are a memory management agent that helps determine if cached memories are sufficient."
            answer = self._generate_chat_response(system_msg, prompt, temperature=0.1)
    ...
    def search_memory(self, conversation):
        ...
        Which of these memory abstractions are most relevant to the question? 
...
請判斷哪些記憶摘要與這個問題最相關？  
請列出應該被擴展以檢索更詳細記憶的摘要編號，  
並簡要說明你對每個選擇的理由。
"""
        system_msg = "You are a memory management agent that helps retrieve relevant memories." if self.language == "eng" else "你是一個記憶管理助手，負責協助擷取相關的記憶內容。"
        user_msg = prompt if self.language == "eng" else prompt_zh

        to_expand = []

        to_expand = top_memories[:3]  
        ...
        # print(top_memories)
        # print("------------------")
        # print("-------second order memories-----------")
        # print(second_order_results)
        # print("------------------")
        # print("-------first order memories-----------")
        ...
        Create a concise summary of the information from these memories that is most relevant to the question.
        Focus on providing helpful context that would assist in answering this question effectively.
...
請針對這些記憶中**與這個問題最相關的資訊**，撰寫一份簡潔摘要。  
重點是提供有幫助的背景脈絡，以利有效回答問題。  
當日期、姓名或具體事實很重要時，請務必包含這些細節。
"""
        system_msg_final = "You are a memory management agent that helps retrieve and summarize relevant memories." if self.language == "eng" else "你是一個記憶管理助手，負責協助擷取並摘要相關的記憶內容。"
        user_msg_final = final_prompt if self.language == "eng" else final_prompt_zh

        memory_summary = self._generate_chat_response(system_msg_final, user_msg_final, temperature=0.3)

        # Update cache with the latest question and memory summary
        self.last_conversation = conversation
        self.last_memory_summary = memory_summary

        return memory_summary, top_memories, second_order_results, first_order_results

class HierarchicalMemoryModule():
    def __init__(self, attribute_path, extraction_model, dataset, model="gpt-4o", person_name="", 
                 model_instance=None, tokenizer=None, language: str = "zh"):
        """
        Initialize the hierarchical memory module.
        
        Args:
            attribute_path: Path to the attribute/memory bank JSON file
            extraction_model: Model used for memory extraction
            model: Model to use for LLM calls (default: "gpt-4o")
            person_name: Name of the person/character
            model_instance: Pre-loaded open-source model instance (optional)
            tokenizer: Pre-loaded tokenizer for open-source model (optional)
            language: Language for prompts ("zh" or "eng")
        """
        ...
        embeddings_cache_folder = f"./IMPersona/data/{dataset}/{person_name}/{extraction_model}"
        ...
        cache_filename = "memory_bank_embeddings.json"
        self._embeddings_cache_path = f"{embeddings_cache_folder}/{cache_filename}"
...
class MemoryModule():
    def __init__(self, attribute_path, extraction_model, dataset, person_name=""):
        assert attribute_path, "Attribute path is required"
        assert os.path.exists(attribute_path), "Attribute path does not exist"

        # Ensure cache directory exists
        os.makedirs(f"./data/{dataset}/{person_name}/cache/{extraction_model}", exist_ok=True)
        ...
        base_filename = os.path.basename(attribute_path)
        cache_filename = "memory_bank_embeddings.json"
        self._embeddings_cache_path = f"./data/{dataset}/{person_name}/cache/{extraction_model}/{cache_filename}"
    ...
    def search_memory(self, query: str, top_k: int = 6, recency_weight: float = 0.15) -> str:
        ...
        print("using simple model memory search")
